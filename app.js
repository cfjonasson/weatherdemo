(() => {
  ...
  // Förväntade element från nya index.html
  const els = {
    city: $("city"),
    refresh: $("refresh"),
    updated: $("updated"),
  ...
  // Om du råkar ha gammal index.html (utan select + metrics),
  // visa ett tydligt fel så du vet vad som saknas.
  ...
  if (missing.length) {
    ...
    const msg =
      "Saknar HTML-element: " +
      missing.join(", ") +
      ". Kontrollera att index.html är den nya versionen (med select#city och spans #temp/#feels/#wind).";
    // Försök visa på sidan om möjligt
    document.body.insertAdjacentHTML(
      "afterbegin",
      `<div style="padding:12px;margin:12px;border:2px solid #f33;border-radius:10px;font-family:system-ui;">
         <strong>Fel:</strong> ${msg}
       </div>`
    );
    return;
  }
  ...
  console.log("Loaded cities:", CITIES.map((c) => c.name));
  ...
  async function loadWeatherForCity(city) {
    ...
    // Visa vald stad + laddstatus
    els.condition.textContent = `Hämtar väder för ${city.name}…`;
    els.updated.textContent = "Senast uppdaterad: —";
    ...
    try {
      ...
      els.condition.textContent = `${city.name}: ${codeToText(cur.weather_code)}`;
  ...
  function init() {
    ...
    // Välj sparad stad om möjligt
    const saved = loadSelectedCity();
    const initial = CITIES.some((c) => c.id === saved) ? saved : "stockholm";
